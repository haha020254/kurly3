    ...
    return (
        ...
        const HeaderComponent = ({loginState, memberState}) => {
            ...
            const onClickPopupClose=(e)=>{
                ...
                setCookie('popup20220809', 'no', 1);
                //setCookie('popup20220809_2', 'no', 1);
                //setCookie('haha020254', '13579', 1);
            }

            const popupOpen=()=>{
                const result = getCookie('popup20220809');
                if(result !== 'no'){
                    setPopup(true);
                }
            }
        ...
        const LoginComponent = ({memberState}) =>{
            ...
            const axiosEvent=()=>{
                ...
                .then((res)=>{
                    ...
                    location.href = './';
                    //location.href = 'http://haha020254.dothome.co.kr/kurly2_/'
                })
                .catch((err)=>{
                    console.log(err);
                })
                    ...
                    <div className="container">
                        ...
                        <div className="content">
                            <form name='login_form' id='loginForm' method="post" autoComplete='off'>
                                <ul>
                                    <li>
                                        <input onChange={onChangeId} type="text" name="id" id="id" value={userId} placeholder='아이디를 입력해주세요'/>
                                    </li>
                                    <li>
                                        <input onChange={onChangePw} type="password" name="pw" id="pw" value={userPw} placeholder='비밀번호를 입력해주세요'/>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <button onClick={onClickLoginBtn} type='button' className='login-btn'>로그인</button>
                                    </li>
                                    <li>
                                        <button onClick={onClickJoin} type='button' className='join-btn'>회원가입</button>
                                    </li>
                                </ul>
                            </form>
                        </div>
        ...
        const MainComponent = ({modal, modalShowFn}) => {
            ...
            const MemberComponent = ({modalShowFn, 이용약관}) => {
                ...
                const onCompletePost=(data)=>{
                    setField({...field, isShowAddress:true, 주소1:data.roadAddress})
                }
                ...
                const onSubmitMember=(e)=>{
                    ...
                    if(아이디==='' || 비밀번호==='' || 비밀번호확인==='' || 이름==='' || 휴대폰==='' || 이메일==='' || 주소1==='' || 주소2==='' || 이용약관동의.length<3 || 아이디중복확인===false || 이메일확인=== false || 인증번호확인Ok===false){
                        ...
                        if(cnt<3){
                            ...
                            formData.append('birthday', `${년}-${월}-${일}`);
